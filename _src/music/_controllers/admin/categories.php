...
class Cadmin_categories extends controllers {
	...
	public function ajax_delete() {
		...
		$sql = $db->delete("categories","`cat_key` = '".$key."'",1);
		
		$data = array (
			"cat_parent" => 0
		);		
		$sql = $db->update("categories", $data, "`cat_parent`='".$key."'");
	...
	public function ajax_edit() {
		...
		if($helper->error!="") {
			...
			$sql = $db->update("categories", $data, "`cat_key`='".$cat_key."'",1);
		}				
